...
function buildQuery(keywords: string[], location: string, language: string, platform: Platform): string {
...
function extractDisplayName(title: string, platform: Platform): string {
  if (!title) return '';
  const t = title.trim();
  switch (platform) {
    case 'twitter':
...
// SerpAPI sometimes includes "123K followers" in titles/snippets

function estimateFollowers(title: string, snippet: string): number | null {
